...
function buildChatPanel() {
  ...
  gifInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") gifBtn.click();
  });
  return panel;
}
function appendGifResult(url, container, inputToClear) {
  const img = document.createElement("img");
  img.src = url;
...
function createAndStartRecognition() {
  ...
  rec.onresult = (event) => {
    ...
    const overlayInput = document.getElementById("__wt_chat_input__");
  ...
  try {
    ...
    if (voiceActive) setTimeout(() => createAndStartRecognition(), 300);
  }
}
function updateVoiceBtnState(active) {
  const btn = document.getElementById("__wt_mic_btn__");
  if (!btn) return;
